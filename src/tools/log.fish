...
function warning --description 'Log the warning to a file and to the screen' -a message
    # Create the log file if it doesn't exist
    test -z "$log"; and set -gx log /dev/null
    not test -e "$log"; and touch "$log"
    set -U __WARNINGS__
...
function success --description 'Log the info to a file and to the screen' -a message
    # Create the log file if it doesn't exist
    test -z "$log"; and set -gx log /dev/null
    not test -e "$log"; and touch "$log"
    set_color brgreen
