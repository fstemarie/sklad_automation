...
set src "/l/containers/nodered" # Variable qui contient le chemin du dossier de sauvegarde
...
or source /data/automation/tools/delete_old_backups.fish # inclut le fichier tools.fish pour utiliser les fonctions d'outils génériques
...
info "Vérification de l'existence du dossier source $src"
if test -d "$src"
...
info "Vérification de l'existence du dossier de destination $dst"
if test -d "$dst"
...
delete_old_backups "$dst" "nodered.*.tar.zst" $nb_max
if test $status -eq 0
